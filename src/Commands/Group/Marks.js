...
module.exports = class extends Command {
    constructor(...args) {
        super(...args, {
            aliases: ['am', 'gxp'],
            description: 'Give a user or users a specified amount of marks',
            category: 'Group',
            usage: 'addmarks <points> <user(s)>'
        });
    }
    ...
    async run(message, args) {
        ...
        if (points >= min && points <= max) {
            ...
            let update = await User.updateMany({ discordId: newUsers }, { $inc: { points } });

            const embed = new MessageEmbed()
                .setTitle(`Marks have been Added`)
                .setColor('#87f04a')
